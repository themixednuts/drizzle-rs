...
use drivers::rusqlite::RusqliteConnection;
use drivers::{Connection, DbRow, DriverError, PreparedStatement, SQLiteValue, Transaction};
use rusqlite::Connection as NativeRusqliteConnection;
use std::borrow::Cow;

fn create_in_memory_db() -> RusqliteConnection<'static> {
    let conn =
        NativeRusqliteConnection::open_in_memory().expect("Failed to open in-memory database");
    RusqliteConnection::new(conn)
}
...
fn test_data_types_real_blob() -> Result<(), DriverError> {
    ...
    assert_eq!(rows[0].get(1)?, SQLiteValue::Blob(Cow::Owned(image_data)));

    Ok(())
}

// TODO: Add tests for transactions (commit, rollback)
// TODO: Add tests for BLOB and REAL types
