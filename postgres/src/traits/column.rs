...
pub trait PostgresColumnInfo: SQLColumnInfo + Any {
    ...
    /// `true` = GENERATED ALWAYS, `false` = GENERATED BY DEFAULT.
    /// Only meaningful when `is_generated_identity()` returns `true`.
...
impl<T: PostgresColumnInfo> PostgresColumnInfo for &'static T {
    ...
    fn is_identity_always(&self) -> bool {
        <T as PostgresColumnInfo>::is_identity_always(*self)
    }
