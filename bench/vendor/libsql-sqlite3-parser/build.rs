...
fn main() -> Result<()> {
    ...
    // compile rlemon:
    {
        // VENDORED PATCH (drizzle-rs) -- the only change from the published 0.13.0.
        //
        // `-o` places the *executable* in OUT_DIR, but MSVC's cl.exe also writes an intermediate
        // `lemon.obj` next to its current directory, which for a build script is the crate's source
        // directory. Under the registry that directory is shared by every build on the machine, so
        // two cargo processes compiling this crate at once -- a `cargo build` alongside
        // rust-analyzer, say -- race on one path and whichever loses fails the assert below. It is
        // intermittent, it only ever bites the first build, and it does not exist on gcc or clang
        // because neither leaves an object file behind.
        //
        // `/Fo` sends that intermediate to OUT_DIR, which is per-build. Guarded on the compiler
        // being MSVC-like so the gcc and clang paths keep the exact command they had.
        let compiler = Build::new()
            .target(&env::var("HOST").unwrap())
            .get_compiler();
        let mut cmd = compiler.to_command();
        if compiler.is_like_msvc() {
            cmd.arg(format!("/Fo{}", out_path.join("lemon.obj").display()));
        }
        assert!(cmd.arg("-o").arg(rlemon.clone()).arg(rlemon_src).status()?.success());
    }
