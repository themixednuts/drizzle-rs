//! Toasty PostgreSQL backend.
//!
//! Models map onto the shared Northwind-micro schema that the benchmark runner
//! seeds; toasty is told about the tables but never asked to create them
//! (`push_schema()` / `reset_db()` are deliberately never called, so `connect()`
//! issues no DDL).
//!
//! Deviations from the canonical query catalog, all forced by toasty 0.9's
//! query surface and declared in the target spec's `sql_variant`:
//!
//! * No user-reachable JOIN, so `/employee-with-recipient`,
//!   `/product-with-supplier` and `/order-with-details-and-products` load their
//!   related rows with additional round trips instead of one joined statement.
//! * No `GROUP BY` / `SUM`, so `/orders-with-details` fetches the order page and
//!   the matching `order_details` id range and aggregates in Rust — the same
//!   shape the built-in Turso targets use.
//!
//! Nothing here does *less* work than the other targets; where toasty cannot
//! express a join it pays for extra round trips.
...
const SEED_CUSTOMERS: i32 = 10_000;
const SEED_EMPLOYEES: i32 = 200;
const SEED_ORDERS: i32 = 50_000;
const SEED_SUPPLIERS: i32 = 1_000;
const SEED_PRODUCTS: i32 = 5_000;
...
// ---------------------------------------------------------------------------
// Responses (identical JSON to bench/targets/rust-pg-orms)
// ---------------------------------------------------------------------------
...
fn opt_date_string(date: Option<Date>) -> Option<String> {
...
impl From<Employee> for EmployeeResponse {
    fn from(row: Employee) -> Self {
        Self {
            ...
            birth_date: date_string(row.birth_date),
            hire_date: date_string(row.hire_date),
            address: row.address,
            city: row.city,
            postal_code: row.postal_code,
            country: row.country,
...
pub async fn serve(seed: u64) -> Result<(), DynError> {
    crate::common::seed_postgres(seed)?;

    let pool_size = configured_pool_size(8);
    let url = crate::common::normalize_database_url();
...
/// Open every pooled connection concurrently before announcing readiness, so
...
async fn customer_by_id(
    ...
    let mut db = handle(&state);
    let id = params.id_mod(SEED_CUSTOMERS);
...
async fn supplier_by_id(
    ...
    let mut db = handle(&state);
    let id = params.id_mod(SEED_SUPPLIERS);
...
async fn employee_with_recipient(
    ...
    let mut db = handle(&state);
    let id = params.id_mod(SEED_EMPLOYEES);
...
async fn product_with_supplier(
    ...
    let mut db = handle(&state);
    let id = params.id_mod(SEED_PRODUCTS);
...
async fn orders_with_details(
    ...
    for order in orders {
        ...
        out.push(OrderWithDetailsResponse {
            id: order.id,
            shipped_date: opt_date_string(order.shipped_date),
...
/// Order ids are dense and the page is ordered by id, so a `BETWEEN` over the
/// page's id range fetches exactly the details for that page with a statement
/// shape that stays constant (two bound params) across pages.
...
async fn order_with_details(
    ...
    let mut db = handle(&state);
    let id = params.id_mod(SEED_ORDERS);
    ...
    Ok(Json(
        orders
            ...
            .map(|order| SingleOrderWithDetailsResponse {
                id: order.id,
                order_date: date_string(order.order_date),
                required_date: date_string(order.required_date),
                shipped_date: opt_date_string(order.shipped_date),
...
async fn search_customer(
    ...
    let mut db = handle(&state);
    let rows = Customer::filter(Customer::fields().company_name().ilike(params.pattern()))
        .exec(&mut db)
        .await
        .map_err(internal)?;
    Ok(Json(rows.into_iter().map(Into::into).collect()))
...
async fn search_product(
    ...
    let mut db = handle(&state);
    let rows = Product::filter(Product::fields().name().ilike(params.pattern()))
        .exec(&mut db)
        .await
        .map_err(internal)?;
    Ok(Json(rows.into_iter().map(Into::into).collect()))
