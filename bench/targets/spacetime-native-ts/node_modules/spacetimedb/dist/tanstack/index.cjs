...
// src/tanstack/SpacetimeDBQueryClient.ts
var queryRegistry = /* @__PURE__ */ new Map();
function spacetimeDBQuery(queryOrSkip) {
  if (queryOrSkip === "skip") {
    return {
      queryKey: ["spacetimedb", "", "skip"],
      staleTime: Infinity,
      enabled: false
    };
  }
  ...
  return {
    queryKey: ["spacetimedb", accessorName, querySql],
    staleTime: Infinity
  };
}
var SpacetimeDBQueryClient = class {
  connection = null;
  queryClient = null;
  ...
  connect(queryClient) {
    ...
    this.cacheUnsubscribe = queryClient.getQueryCache().subscribe((event) => {
      if (event.type === "removed" && event.query.queryKey[0] === "spacetimedb") {
  ...
  queryFn = async ({
    ...
    const [prefix, accessorName, querySql] = queryKey;
  ...
  getTableData(tableInstance, whereExpr) {
  ...
  setupSubscription(queryKey, accessorName, querySql, whereExpr) {
    if (!this.connection) {
    ...
    return new Promise((resolve) => {
      ...
      const handle = this.connection.subscriptionBuilder().onApplied(() => {
        const sub = this.subscriptions.get(keyStr);
        if (sub) {
          sub.applied = true;
        }
...
function useSpacetimeDBQuery(queryOrSkip, options) {
  const queryOptions = queryOrSkip === "skip" ? spacetimeDBQuery("skip") : spacetimeDBQuery(queryOrSkip);
...
function useSpacetimeDBSuspenseQuery(query, options) {
  const queryOptions = spacetimeDBQuery(query);
  const q = reactQuery.useSuspenseQuery({
    ...queryOptions,
    ...options
  });
  return [q.data, false, q];
}
var SpacetimeDBContext = React.createContext(
  void 0
);
...
function SpacetimeDBProvider({
  ...
  const fallbackStateRef = React__namespace.useRef({
    isActive: false,
    identity: void 0,
    token: void 0,
    connectionId: ConnectionId.random(),
  ...
  const getServerSnapshot = React__namespace.useCallback(
    () => fallbackStateRef.current,
    []
  );
  const state = React__namespace.useSyncExternalStore(
    subscribe,
    getSnapshot,
    getServerSnapshot
  );
  const getConnection = React__namespace.useCallback(
...
exports.SpacetimeDBQueryClient = SpacetimeDBQueryClient;
exports.spacetimeDBQuery = spacetimeDBQuery;
exports.useReducer = useReducer;
exports.useSpacetimeDB = useSpacetimeDB;
exports.useSpacetimeDBQuery = useSpacetimeDBQuery;
exports.useSpacetimeDBSuspenseQuery = useSpacetimeDBSuspenseQuery;
