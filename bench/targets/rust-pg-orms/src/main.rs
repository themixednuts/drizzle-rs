...
/// so the default matches the single-threaded Bun targets rather than
/// silently taking every core the way `#[tokio::main]` did.
fn configured_workers(default: usize) -> usize {
...
fn main() -> Result<(), DynError> {
    let workers = configured_workers(1);
    // Diesel runs its blocking queries on the blocking pool; bounding it to the
    // connection pool keeps its in-flight concurrency equal to the async
    // drivers' pool-limited concurrency instead of spawning unbounded threads.
    let max_blocking = configured_pool_size(8).max(1);
    ...
    let state = match target.as_str() {
        "sqlx-pg" => {
            let pool = PgPoolOptions::new()
                ...
                .min_connections(pool_size as u32)
                .connect(&database_url)
                .await?;
            // Force every pooled connection open before we announce readiness so
            // the measured window never pays for a TCP + startup handshake.
            let mut warm = Vec::with_capacity(pool_size);
        ...
        // DieselPool::new establishes every connection eagerly.
        ...
        "seaorm-pg" => {
            ...
            options
                .max_connections(pool_size as u32)
                .min_connections(pool_size as u32)
                // sqlx statement logging is on by default and lands in the hot path.
                .sqlx_logging(false);
            let conn = Database::connect(options).await?;
            let conn = Arc::new(conn);
            warm_sea_pool(&conn, pool_size).await;
...
/// Open `size` SeaORM connections concurrently so the pool is hot before the
/// measured window starts (sea-orm's `min_connections` only fills lazily).
async fn warm_sea_pool(conn: &Arc<DatabaseConnection>, size: usize) {
    let mut handles = Vec::with_capacity(size);
    for _ in 0..size {
        let conn = Arc::clone(conn);
        handles.push(tokio::spawn(async move {
            conn.query_one_raw(Statement::from_string(DbBackend::Postgres, "SELECT 1"))
                .await
        }));
    }
...
/// Same two round trips (base row, then details) that the sqlx and diesel
/// targets issue, with the nesting done in Rust. This route family is more than
/// half the request mix, so a server-side `json_build_object` shortcut here
/// would not be comparable work.
async fn sea_order_with_details(
    conn: Arc<DatabaseConnection>,
    id: i32,
    with_products: bool,
) -> HttpResult {
    let order_rows = sea_query_all(&conn, SELECT_ORDER_BASE, vec![id.into()]).await?;
    let mut orders = Vec::with_capacity(order_rows.len());
    for row in &order_rows {
        orders.push(sea_order_base(row).map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?);
    }

    if with_products {
        let detail_rows =
            sea_query_all(&conn, SELECT_ORDER_DETAIL_PRODUCTS, vec![id.into()]).await?;
        let mut details = Vec::with_capacity(detail_rows.len());
        for row in &detail_rows {
            details.push(
                sea_order_detail_product(row).map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?,
            );
        }
        json(
            orders
        ...
        let detail_rows = sea_query_all(&conn, SELECT_ORDER_DETAILS, vec![id.into()]).await?;
        let mut details = Vec::with_capacity(detail_rows.len());
        for row in &detail_rows {
            details.push(sea_order_detail(row).map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?);
        }
        json(
            orders
                .into_iter()
...
async fn sea_query_all(
    conn: &Arc<DatabaseConnection>,
    sql: &str,
    values: Vec<sea_orm::Value>,
) -> Result<Vec<sea_orm::QueryResult>, StatusCode> {
...
fn sea_order_base(row: &sea_orm::QueryResult) -> Result<OrderBase, sea_orm::DbErr> {
    Ok(OrderBase {
        id: row.try_get("", "id")?,
...
fn sea_order_detail(row: &sea_orm::QueryResult) -> Result<OrderDetailResponse, sea_orm::DbErr> {
...
fn sea_order_detail_product(
    row: &sea_orm::QueryResult,
) -> Result<OrderDetailProductResponse, sea_orm::DbErr> {
