...
// Same backpressure contract as the other two Bun targets: at most pool-size
// queries in flight, the rest queue in-process instead of piling onto the pool.
...
// window never pays for a TCP+startup handshake or a lazy engine init.
await Promise.all(
  Array.from({ length: poolSize() }, () => dbQuery(() => prisma.$queryRaw`SELECT 1`)),
);
