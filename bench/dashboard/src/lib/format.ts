...
/**
 * Run ids look like `20260305T180000Z_abc1234_throughput-http`. Within a benchmark set every
 * shard shares the commit and suite, so the timestamp is what tells the CI jobs apart.
 */
export function runStamp(runId: string): string {
	const stamp = runId.split('_')[0] ?? runId;
...
/** Which CI VM a row came from: OS plus the shard's run timestamp. */
export function shardLabel(os: string, runId: string): string {
	return `${os} / ${runStamp(runId)}`;
}
