...
fn generate_create_statements_method(fields: &[(&syn::Ident, &syn::Type)]) -> TokenStream {
    // Generate match blocks for each field using the trait objects from SQLSchemaType
    let field_matches = fields.iter().map(|(name, ty)| {
        quote! {
            match <#ty as ::drizzle_rs::core::SQLSchema<'_, ::drizzle_rs::core::SQLSchemaType, ::drizzle_rs::sqlite::SQLiteValue<'_>>>::TYPE {
                ::drizzle_rs::core::SQLSchemaType::Table(table_info) => {
                    let table_name = table_info.name();
                    let table_sql = self.#name.sql().sql();
                    tables.push((table_name, table_sql, table_info));
                }
                ::drizzle_rs::core::SQLSchemaType::Index(index_info) => {
                    let index_sql = self.#name.sql().sql();
