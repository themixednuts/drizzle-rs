...
fn generate_constructor_param(info: &FieldInfo) -> (TokenStream, TokenStream) {
    ...
    match category {
        // JSON payloads bind as the column's `json` or `jsonb` through
        // `Json<Payload>`; nothing is implemented on the payload type.
        _ if info.is_json_payload() => {
            let value = info.json_insert_value(&quote!(#field_name));
            (
