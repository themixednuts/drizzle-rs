...
fn generate_select_field_conversion(idx: &TokenStream, info: &FieldInfo) -> TokenStream {
    ...
    // PostgresEnum owns its storage codec, including native and integer-backed enums.
    if info.is_pgenum {
        return quote! {
            #name: <#field_type as drizzle::core::FromDrizzleRow<drizzle::postgres::Row>>
                ::from_row_at(row, #idx)?,
        };
    }

    // Custom types use DrizzlePostgresColumn for deserialization.
    if info.is_custom_type {
        if info.is_nullable {
            return quote! {
                #name: {
                    let is_null: bool = row.try_get::<_, Option<i32>>(#idx)
                        .map(|v| v.is_none())
                        .unwrap_or(true);
                    if is_null {
                        None
    ...
    // Handle legacy enums - check column type for storage format
    // - is_enum with INTEGER column: stored as integer, use try_into()
    // - is_enum with TEXT column: stored as text, use parse()
    if info.is_enum {
        let is_integer_enum = is_integer_column(&info.column_type);
        ...
        if is_integer_enum {
            ...
            // Text-stored enum (non-native): read as String and use parse()
            if info.is_nullable {
                quote! {
                    #name: {
...
fn generate_partial_field_conversion(idx: usize, info: &FieldInfo) -> TokenStream {
    ...
    if info.is_pgenum {
        return quote! {
            #name: <#base_type as drizzle::core::FromDrizzleRow<drizzle::postgres::Row>>
...
fn null_probe_type(info: &FieldInfo) -> TokenStream {
    if info.is_custom_type {
        // Custom types: probe with String since we don't know the storage format
        return quote!(String);
    }
    if info.is_enum || info.is_pgenum {
        if is_integer_column(&info.column_type) {
            quote!(i32)
        } else if info.is_pgenum {
...
#[cfg(any(feature = "postgres-sync", feature = "tokio-postgres"))]
pub fn generate_all_driver_impls(ctx: &MacroContext) -> TokenStream {
    let drizzle_error = paths::core::drizzle_error();
    ...
    let null_probe_impl = field_infos.first().map_or_else(
        || quote! {},
        |first_field| {
...
pub(crate) fn generate_all_driver_impls(_ctx: &super::context::MacroContext) -> TokenStream {
    TokenStream::new()
}
