...
fn generate_model_trait_impls(ctx: &MacroContext, _column_zst_idents: &[Ident]) -> TokenStream {
    ...
    let partial_impl = quote! {
        impl<'a> #sql_model<'a, #mysql_value<'a>> for #select_model_partial {
            fn columns(&self) -> ::std::borrow::Cow<'static, [#column_ref]> {
                ::std::borrow::Cow::Borrowed(<#struct_ident as #drizzle_table>::TABLE_REF.columns)
            }

            fn values(&self) -> #sql<'a, #mysql_value<'a>> {
                #sql::empty()
            }
        }

        impl<'a> #to_sql<'a, #mysql_value<'a>> for #select_model_partial {
    ...
    quote! {
        ...
        impl<'a> #sql_model<'a, #mysql_value<'a>> for #select_model {
            fn columns(&self) -> ::std::borrow::Cow<'static, [#column_ref]> {
                ::std::borrow::Cow::Borrowed(<#struct_ident as #drizzle_table>::TABLE_REF.columns)
            }
        ...
        impl<'a> #to_sql<'a, #mysql_value<'a>> for #select_model {
            fn to_sql(&self) -> #sql<'a, #mysql_value<'a>> {
                #sql_model::values(self)
            }
        }

        impl<'a> #sql_partial<'a, #mysql_value<'a>> for #select_model {
            type Partial = #select_model_partial;
        }

        impl<'a> #sql_model<'a, #mysql_value<'a>> for #update_model<'a, #non_empty_marker> {
            fn columns(&self) -> ::std::borrow::Cow<'static, [#column_ref]> {
                ::std::borrow::Cow::Borrowed(<#struct_ident as #drizzle_table>::TABLE_REF.columns)
            }
