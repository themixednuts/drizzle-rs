...
impl<'tx, 'q, Schema, M>
    TransactionBuilder<'tx, Schema, SelectBuilder<'q, Schema, SelectInitial, (), M>, SelectInitial>
{
    #[inline]
    pub fn from<T>(
        self,
...
macro_rules! impl_tx_select_methods {
    ($($state:ty => [$($method:ident),* $(,)?]),+ $(,)?) => {
        $( impl<'tx, 'q, Schema, T, M, R, G>
            TransactionBuilder<'tx, Schema, SelectBuilder<'q, Schema, $state, T, M, R, G>, $state>
        { $( impl_tx_select_methods!(@method $method); )* } )+
    };
    (@method r#where) => {
        #[inline]
        pub fn r#where<E>(self, condition: E) -> TransactionBuilder<'tx, Schema, SelectBuilder<'q, Schema, SelectWhereSet, T, M, R, G>, SelectWhereSet>
    ...
    (@method group_by) => {
        pub fn group_by<Gr>(self, columns: Gr) -> TransactionBuilder<'tx, Schema, SelectBuilder<'q, Schema, SelectGroupSet, T, M, R, Gr::Columns>, SelectGroupSet>
    ...
    (@method having) => {
        pub fn having<E>(self, condition: E) -> TransactionBuilder<'tx, Schema, SelectBuilder<'q, Schema, SelectGroupSet, T, M, R, G>, SelectGroupSet>
        where E: drizzle_core::expr::Expr<'q, SQLiteValue<'q>>, E::SQLType: drizzle_core::types::BooleanLike,
        { let builder = self.builder.having(condition); TransactionBuilder { transaction: self.transaction, builder, _phantom: PhantomData } }
    };
    (@method order_by) => {
        pub fn order_by<TOrderBy>(self, expressions: TOrderBy) -> TransactionBuilder<'tx, Schema, SelectBuilder<'q, Schema, SelectOrderSet, T, M, R, G>, SelectOrderSet>
    ...
    (@method limit) => {
        #[inline]
        pub fn limit(self, limit: usize) -> TransactionBuilder<'tx, Schema, SelectBuilder<'q, Schema, SelectLimitSet, T, M, R, G>, SelectLimitSet>
        { let builder = self.builder.limit(limit); TransactionBuilder { transaction: self.transaction, builder, _phantom: PhantomData } }
    };
    (@method offset) => {
        pub fn offset(self, offset: usize) -> TransactionBuilder<'tx, Schema, SelectBuilder<'q, Schema, SelectOffsetSet, T, M, R, G>, SelectOffsetSet>
        { let builder = self.builder.offset(offset); TransactionBuilder { transaction: self.transaction, builder, _phantom: PhantomData } }
    };
    (@method join) => {
        #[inline]
        pub fn join<J: drizzle_sqlite::helpers::JoinArg<'q, T>>(self, arg: J) -> TransactionBuilder<'tx, Schema, SelectBuilder<'q, Schema, SelectJoinSet, J::JoinedTable, <M as drizzle_core::ScopePush<J::JoinedTable>>::Out, <M as drizzle_core::AfterJoin<R, J::JoinedTable>>::NewRow, G>, SelectJoinSet>
        ...
        transaction_builder_join_impl!('q; 'tx);
    };
}
