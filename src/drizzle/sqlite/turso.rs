...
impl<Schema> Drizzle<Schema> {
    ...
    pub async fn migrate(&self, migrations_dir: &std::path::Path) -> drizzle_core::error::Result<usize> {
    ...
    pub async fn migrate_with_config(&self, config_path: &std::path::Path) -> drizzle_core::error::Result<usize> {
        ...
        // Create migrations table
        self.conn.execute(&migrator.create_migrations_table_sql(), ()).await?;
        ...
        for migration in pending {
            ...
            self.conn.execute(&migrator.record_migration_sql(&migration.tag), ()).await?;
        }
