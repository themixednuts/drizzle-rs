...
use crate::prelude::*;
use crate::values::{OwnedSQLiteValue, SQLiteValue, SQLiteValueRef};
use drizzle_core::conv::checked_float_to_int;
use drizzle_core::error::DrizzleError;

/// SQLite affinity metadata owned by a Drizzle SQL type marker.
pub trait SQLiteAffinity: drizzle_core::types::DataType {
    /// SQLite affinity spelling used in DDL and schema metadata.
    const SQL_TYPE: &'static str;
}

macro_rules! impl_sqlite_affinity {
    ($($marker:ty => $sql_type:literal),+ $(,)?) => {
        $(
            impl SQLiteAffinity for $marker {
...
impl_sqlite_affinity! {
    crate::types::Integer => "INTEGER",
    crate::types::Text => "TEXT",
    crate::types::Blob => "BLOB",
    crate::types::Real => "REAL",
    crate::types::Numeric => "NUMERIC",
    crate::types::Any => "ANY",
}
...
/// # Associated Types
///
/// - `SQLType`: The Drizzle SQL type marker used for typed expressions and,
///   through [`SQLiteAffinity`], DDL/schema affinity metadata.
///
/// # Required Methods
///
...
pub trait DrizzleSQLiteColumn: Sized {
    ...
    /// SQLite affinity spelling retained for source compatibility with custom
    /// column implementations.
    ///
    /// Schema generation uses [`SQLType`](Self::SQLType) and
    ...
    /// Decode a value emitted by SQLite's storage-class-preserving JSON query
    /// projection.
    ///
    /// # Errors
    ///
    /// Returns [`DrizzleError::ConversionError`] when the JSON value cannot be
    /// represented by this column's SQLite storage codec.
    #[cfg(feature = "query")]
    fn decode_json(value: &serde_json::Value) -> Result<Self, DrizzleError> {
        let value = decode_projected_sqlite_value(value)?;
        let value = value.as_value();
        Self::decode(value.as_ref())
    }
}

/// Decode the tagged value emitted for a storage-codec candidate in SQLite's
/// JSON query projection.
///
/// The tag carries `typeof(column)` alongside the value, so a TEXT cell in a
/// BLOB-affinity column remains TEXT and is rejected by codecs that require
/// BLOB instead of being silently interpreted as hexadecimal bytes.
///
/// # Errors
///
/// Returns [`DrizzleError::ConversionError`] for malformed tags, values that do
/// not match their tagged SQLite storage class, or invalid BLOB hex.
#[cfg(feature = "query")]
pub fn decode_projected_sqlite_value(
    value: &serde_json::Value,
) -> Result<OwnedSQLiteValue, DrizzleError> {
    if value.is_null() {
        return Ok(OwnedSQLiteValue::Null);
    }

    let object = value.as_object().ok_or_else(|| {
        DrizzleError::ConversionError("SQLite query value is missing its storage tag".into())
    })?;
    let storage = object
        .get("$drizzle_storage")
        .and_then(serde_json::Value::as_str)
        .ok_or_else(|| {
            DrizzleError::ConversionError("SQLite query value has no valid $drizzle_storage".into())
        })?;
    let value = object.get("$drizzle_value").ok_or_else(|| {
        DrizzleError::ConversionError("SQLite query value has no $drizzle_value".into())
    })?;

    match storage {
        "integer" => value
            .as_i64()
            .map(OwnedSQLiteValue::Integer)
            .ok_or_else(|| {
                DrizzleError::ConversionError(
                    "SQLite INTEGER query value is not a JSON integer".into(),
                )
            }),
        "real" => value.as_f64().map(OwnedSQLiteValue::Real).ok_or_else(|| {
            DrizzleError::ConversionError("SQLite REAL query value is not a JSON number".into())
        }),
        "text" => value
            .as_str()
            .map(|value| OwnedSQLiteValue::Text(value.to_owned()))
            .ok_or_else(|| {
                DrizzleError::ConversionError("SQLite TEXT query value is not a JSON string".into())
            }),
        "blob" => value
            .as_str()
            .ok_or_else(|| {
                DrizzleError::ConversionError("SQLite BLOB query value is not a hex string".into())
            })
            .and_then(decode_projected_blob)
            .map(|value| OwnedSQLiteValue::Blob(value.into_boxed_slice())),
        other => Err(DrizzleError::ConversionError(
            format!("unknown SQLite query storage class `{other}`").into(),
        )),
    }
}

#[cfg(feature = "query")]
fn decode_projected_blob(value: &str) -> Result<Vec<u8>, DrizzleError> {
    ...
    if !remainder.is_empty() {
        return Err(DrizzleError::ConversionError(
            "hex-encoded SQLite BLOB has odd length".into(),
        ));
    }
    Ok(bytes)
}
...
fn decode_hex_nibble(value: u8) -> Result<u8, DrizzleError> {
    match value {
        ...
        _ => Err(DrizzleError::ConversionError(
            "SQLite BLOB query value contains a non-hex byte".into(),
        )),
    }
}
...
pub trait FromSQLiteValue: Sized {
    ...
    /// # Errors
    ///
    /// Returns [`DrizzleError::ConversionError`] if `value` cannot be
    /// represented as `Self`.
    fn from_sqlite_ref(value: SQLiteValueRef<'_>) -> Result<Self, DrizzleError> {
        match value {
            SQLiteValueRef::Integer(value) => Self::from_sqlite_integer(value),
            SQLiteValueRef::Text(value) => Self::from_sqlite_text(value),
            SQLiteValueRef::Real(value) => Self::from_sqlite_real(value),
            SQLiteValueRef::Blob(value) => Self::from_sqlite_blob(value),
            SQLiteValueRef::Null => Self::from_sqlite_null(),
        }
    }
    ...
    fn from_value_ref(value: ::rusqlite::types::ValueRef<'_>) -> Result<Self, DrizzleError> {
        let value = SQLiteValueRef::try_from_rusqlite_value_ref(value)?;
        Self::from_sqlite_ref(value)
    }
}

impl<T> FromSQLiteValue for T
where
    T: DrizzleSQLiteColumn,
{
    fn from_sqlite_ref(value: SQLiteValueRef<'_>) -> Result<Self, DrizzleError> {
    ...
    fn from_sqlite_integer(value: i64) -> Result<Self, DrizzleError> {
        T::decode(SQLiteValueRef::Integer(value))
    }

    fn from_sqlite_text(value: &str) -> Result<Self, DrizzleError> {
        T::decode(SQLiteValueRef::Text(value))
    }

    fn from_sqlite_real(value: f64) -> Result<Self, DrizzleError> {
        T::decode(SQLiteValueRef::Real(value))
    }

    fn from_sqlite_blob(value: &[u8]) -> Result<Self, DrizzleError> {
        T::decode(SQLiteValueRef::Blob(value))
    }

    fn from_sqlite_null() -> Result<Self, DrizzleError> {
        T::decode(SQLiteValueRef::Null)
    }
}
