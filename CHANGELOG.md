...
- *(sqlite)* read back chrono date-times stored with a space
...
- *(indexes)* support SQLite partial indexes
- *(bench/dashboard)* show both measurements of a run on one row
- *(bench)* run every family per OS, with PostgreSQL 18 everywhere it fits
- *(bench/dashboard)* rebuild the ranking around the field, the ramp and the board
- *(bench)* read latency where the target still keeps up
...
- *(migrations)* add typed SQLite rebuild data plans
...
- *(bench)* describe the per-OS ranking topology
- *(bench)* record the sqlite-ts and libsql families
- *(bench)* record workers.dev hosting decision and dormant-cache behavior

### Fixed

- *(indexes)* preserve partial index metadata
- *(ci)* decode mirrored SpacetimeDB version
- *(ci)* install SpacetimeDB non-interactively
- *(bench/dashboard)* address accessibility audit
- *(sqlite)* enforce safe rebuild execution across adapters
- *(enums)* complete type-owned storage across drivers
- *(sqlite)* make enum storage type-owned across drivers
- *(bench/ci)* make the Windows PostgreSQL start report why it failed
- *(bench/dashboard)* fit the ratio rail to data that clusters
- *(bench)* stop the libsql parser build racing on Windows
...
- *(migrations)* drop the redundant must_use on default_filter
...
- stop rendering the sync PG driver as the database, bump SpacetimeDB to 2.8
- *(dashboard)* keep the hero on every sort, and name the engine class
- *(dashboard)* name what the standings belong to, and drop the win-only rule
- show every OS ranking, and stop boxing the active sort link
- *(dashboard)* keep in-process caches out of the hero
- *(dashboard)* open the ranking on the winner's ramp, not on a filter bar
- *(dashboard)* cut the method page by two thirds and correct what it claimed
- *(dashboard)* cut the method page opening to five claims
- *(dashboard)* stop the page explaining itself before it shows anything
- *(saturation)* find the ceiling by throughput turning over, not by latency
- *(targets)* tune each engine for the workload it is measured on
- *(dashboard)* name both cpuset splits instead of calling them a conflict
- *(dashboard)* scope the ranking to one operating system
- *(runner)* record the database cpuset in topology.cpu_pinning
- *(spec)* end the saturation ramp at 512 VUs and budget-check the sequence
- *(ci)* add db-cpuset input for three-way core isolation
...
- support turso 0.7 and restore transaction panic rollback
