//! Turso driver implementation for remote SQLite databases
//!
//! This module provides async introspection and push functionality
//! for Turso (edge SQLite) connections.
...
use crate::config::credentials::TursoCredentials;
use crate::config::error::ConfigError;
use crate::config::markers::{SqliteDialect, TursoConnection};
use crate::schema::Schema;

impl<S: Schema> Config<S, SqliteDialect, TursoConnection, TursoCredentials> {
    /// Run the CLI with command line arguments for Turso connections.
    ///
    ...
    /// #[tokio::main]
    /// async fn main() {
    ///     TursoConfigBuilder::new("libsql://mydb.turso.io", "auth_token")
    ///         .schema::<AppSchema>()
    ///         .build()
    ///         .run_cli()
    ///         .await;
    ...
    pub async fn migrate(&self) -> Result<(), ConfigError> {
        ...
        println!("🚀 Running migrations on Turso database...");
